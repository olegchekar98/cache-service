...
def get_or_create_payload(
    ...
    existing = _find_by_digest(session, content_digest)
    ...
    payload = Payload(
        ...
        list_1=list(list_1),
        list_2=list(list_2),
        output=_render(session, list_1, list_2),
    )
    session.add(payload)
    try:
        session.commit()
        ...
        session.rollback()
        concurrent = _find_by_digest(session, content_digest)
...
def get_payload(session: Session, payload_id: str) -> Payload | None:
    return session.get(Payload, payload_id)


def _render(session: Session, list_1: Sequence[str], list_2: Sequence[str]) -> str:
    ...
    transformed = transform_all(session, interleaved)
    return SEPARATOR.join(transformed[value] for value in interleaved)


def _find_by_digest(session: Session, content_digest: str) -> Payload | None:
    return session.exec(select(Payload).where(Payload.content_digest == content_digest)).first()
